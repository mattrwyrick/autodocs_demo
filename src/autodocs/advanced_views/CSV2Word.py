...
def create_word_document(request, rules):
    try:
        ...
        return render(request, 'converters/CSV2Word.html')
    ...
    filename = request.POST["file_name"] if request.POST["file_name"] != u"" else "CSV to Word - %s" % datetime.now().isoformat()
